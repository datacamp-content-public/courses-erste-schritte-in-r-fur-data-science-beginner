...
- # diesem Zeichen werden Kommentare gekennzeichnet.
...
`@sct`
```{r}
ex() %>% check_code(c("report.weeksales <- rbind(sell.time, revenue.day)", "rbind(sell.time, revenue.day) -> report.weeksales"), fixed=TRUE, missing_msg="Da stimmt etwas bei dem Erstellen der Matrix nicht. Verwenden Sie bitte die Funktionen und Namen aus der Kontextbeschreibung!") 
...
Fällt Ihnen bei den **Werten des Servicesumsatzes** etwas auf? Welche Aussage bzgl. der Werte der Neukundendaten stimmt **nicht**:
...
Herzlichen Dank für Ihre Teilnahme - ich hoffe, Sie hatten ein gutes Lernerlebnis und Sie konnten etwas für sich mitnehmen. Bitte nehmen Sie an der **folgenden Umfrage teil (!!!)**, die Hauptbestandteil meiner Masterarbeit ist und **nur** auf diese Daten habe ich Zugriff und diese werden in meiner Masterarbeit ausgewertet. 

...
`@pre_exercise_code`
```{r}
Kundendaten <- read.csv2("https://assets.datacamp.com/production/repositories/5035/datasets/4441837babc083eeefaa068de8f2854f79aaa069/PersonID.csv")
```
...
`@sct`
```{r}
ex() %>% check_output(7464, fixed=TRUE, missing_msg="So ist das nicht richtig - vielleicht haben Sie auch nur eine Ausgabefunktion vergessen. Ihr Ziel ist es, dass die Anzahl der Kunden als Ausgabe in der Konsole erscheinen")

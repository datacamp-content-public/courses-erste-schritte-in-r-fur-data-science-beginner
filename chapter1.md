...
`@instructions`
Herr Müller bittet Sie einen Wochenreport mit dem Namen **report.weeksales** für das neue Service- und Supportcenter zu erstellen.

- 1. Ihre Aufgabe ist es eine Matrix aus den Vektoren **sell.time und revenue.day** zu erstellen und der Variablen vom Typ Matrix **report.weeksales** zuzuordnen. Schauen Sie in der Ausgabe der Console, ob es richtig ausgegeben wird.
...
- 2. Naja, lassen Sie sich den erstellten Report (**report.weeksales**) noch einmal ausgeben und korrigieren Sie ihn bitte.
...
success_msg("Ja, genau. Die Funktion str() und summary() sind sehr hilfreich und verschaffen Ihnen einen kompakten Überblick über den Datensatz. Es ist nützlich, die Befehle immer im Hinterkopf zu behalten! Sie haben Ihren Arbeitstag so gut wie geschafft und alle Aufgaben mit höchster Genauigkeit und hohem Engagement absolivert (5.5/6)")
```
